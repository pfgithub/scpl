...
export class ConvertingContext {
	...
	getNamedVariable(name: string): boolean | undefined {
		...
		if(this.namedVariables[name]) {return this.namedVariables[name];}
		if(this.above) {return this.above.getNamedVariable(name);}
		return undefined;
	}
	setNamedVariable(name: string): void {
		// go to the highest this.above and set it there, named variables are global
		if(this.above) {return this.above.setNamedVariable(name);}
		this.namedVariables[name] = true;
	}
	
	getMagicVariable(name: string): {action: Action} | undefined {
		// if this doesn't have it try this.above
		if(this.magicVariables[name]) {return this.magicVariables[name];}
		if(this.above) {return this.above.getMagicVariable(name);}
		return undefined;
	}
	...
	getParserVariable(name: string): AsAble | undefined {
		if(this.parserVariables[name]) {return this.parserVariables[name];}
		if(this.above) {return this.above.getParserVariable(name);}
		return undefined;
	}
	...
	add(action: Action): void {
		...
		if(this.above) {return this.above.add(action);}
